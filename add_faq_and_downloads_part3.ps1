...
function MixCard({ title, subtitle, src, poster, youtube, isFocused, isSectionInView }) {
  ...
  }, [isFocused, isSectionInView])

  const handleEnter = () => {
    const v = videoRef.current
    if (v && isSectionInView) v.play().catch(() => {})
  }
  const handleLeave = () => {
    const v = videoRef.current
    if (v) {
  ...
  return (
    ...
    >
      <video
        ...
        loop
        playsInline
        preload={isSectionInView ? (isFocused ? 'auto' : 'metadata') : 'none'}
...
function MixWheel({ mixes, isSectionInView }) {
        ...
        {mixes.map((mix, i) => {
          ...
          return (
            ...
            >
              ...
              >
                <MixCard {...mix} isFocused={isFocused} isSectionInView={isSectionInView} />
              </div>
            </div>
          )
        })}
...
function DownloadsList() {
  ...
  return (
    <div className="mt-16">
      <h3 className="text-white font-bold uppercase text-xl mb-6">Free Downloads</h3>
      <div className="flex flex-col gap-3">
        {downloads.map((d, i) => (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-between bg-zinc-900 border border-zinc-800 hover:border-blue-500/50 rounded-sm px-5 py-4 transition-colors group"
          >
            <span className="text-white font-semibold text-sm md:text-base">{d.title}</span>
            <span className="text-blue-400 text-sm font-semibold uppercase tracking-wide group-hover:text-blue-300 transition-colors">
              Download &darr;
            </span>
          </a>
        ))}
  ...
  return (
    <section ref={sectionRef} id="sound" className="relative bg-black py-24 md:py-36 px-8 md:px-16 overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        <MixWheel mixes={MIXES} isSectionInView={isInView} />
...
Write-Host "Sound.jsx now shows Free Downloads; FAQ chapter wired in before Booking." -ForegroundColor Green
